...
export function EventsSectionEditor({ initial }) {
  ...
  async function handleUpload(e) {
    ...
    if (mediaPath) {
      await supabase.storage.from("events-section").remove([mediaPath]);
    }

    const ext = extFromFile(file);
    const path = `background-${Date.now()}${ext ? `.${ext}` : ""}`;

    const { error: uploadError } = await supabase.storage.from("events-section").upload(path, file, {
    ...
    const { data: publicUrlData } = supabase.storage.from("events-section").getPublicUrl(path);
    ...
    <div className="mt-6 flex flex-col gap-8">
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          <label className="text-sm text-foreground-muted" htmlFor="eventsTitleBg">
            Title (BG)
          </label>
          <div className="flex gap-2">
            <input
              ...
              placeholder="Предстоящи събития"
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-sm text-foreground-muted" htmlFor="eventsTitleEn">
            Title (EN)
          </label>
          <div className="flex gap-2">
            <input
            ...
            {mediaUrl ? (
              <>
                {mediaType === "video" ? (
                  ...
                  <img src={mediaUrl} alt="Current events section background" className="h-full w-full object-cover" />
                )}
                <button
                  type="button"
