...
export function SignupBackgroundEditor({ initial }) {
  ...
  async function handleUpload(e) {
    ...
    if (mediaPath) {
      await supabase.storage.from("signup").remove([mediaPath]);
    }

    const ext = extFromFile(file);
    const path = `background-${Date.now()}${ext ? `.${ext}` : ""}`;

    const { error: uploadError } = await supabase.storage.from("signup").upload(path, file, {
    ...
    const { data: publicUrlData } = supabase.storage.from("signup").getPublicUrl(path);
          ...
          {mediaUrl ? (
            <>
              {mediaType === "video" ? (
                ...
                <img src={mediaUrl} alt="Current signup background" className="h-full w-full object-cover" />
              )}
              <button
                type="button"
